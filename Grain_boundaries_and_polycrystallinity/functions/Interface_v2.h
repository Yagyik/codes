...
struct GRAIN
{
	...
	int *neigh, *path_stack, *cl_index, *visited, *complete, *boundary, *inPath, *cl_size, *rot_type;
	int **AllNeigh, **nucleo;
...
void print_Interface(SIMDAT *s, DATASIM *d, GRAIN *gr)
{
	...
	for(t=0;t<s->Ato;t++)
	{
		fprintf(fp,"Si%d %f %f %f\n",gr->rot_type[t],d->r[t][0],d->r[t][1],d->r[t][2]);
	}
	fclose(fp);

...
void print_Interface(SIMDAT *s, DATASIM *d, GRAIN *gr)
{
	...
	sprintf(buffer,"%srot_type-ovito-%f-tol-%d.dat",s->output,s->interface_tolerance,gr->fileCounter);
	fp=fopen(buffer,"w");
	fprintf(fp,"%d\n",gr->nucleo[0][0]);
	...
	sprintf(buffer,"%sCl_index-ovito-%f-tol-%d.dat",s->output,s->interface_tolerance,gr->fileCounter);
	fp=fopen(buffer,"w");
	...
	fclose(fp);
	sprintf(buffer,"%sCl_indexNeigh-ovito-%f-tol-%d.dat",s->output,s->interface_tolerance,gr->fileCounter);
	fp=fopen(buffer,"w");
	fprintf(fp,"%d\n",gr->nucleo[0][0]+count);
	fprintf(fp,"random text\n");
	for(t=1;t<=gr->nucleo[0][0];t++)
	...
	sprintf(buffer,"%sCl_size_distro-%f-tol-%d.dat",s->output,s->interface_tolerance,gr->fileCounter);
	fp=fopen(buffer,"w");
	...
	fclose(fp);
	sprintf(buffer,"%sBoundary_ovito-%f-tol-%d.dat",s->output,s->interface_tolerance,gr->fileCounter);
	fp=fopen(buffer,"w");
	fprintf(fp,"%d\n",gr->nucleo[0][0]);
	fprintf(fp,"boundary \n");
	for(t=1;t<=gr->nucleo[0][0];t++)
...
void compute_Interface(SIMDAT *s, DATASIM *d, GRAIN *gr)
{
 ...
 for(i=0;i<s->Ato;i++)
	...
	sprintf(buffer,"%sframe_store-%f-%d.dat",s->output,s->interface_tolerance,gr->fileCounter);
	fp2=fopen(buffer,"w");
	sprintf(buffer,"%slow_dev-%f-tol-%d.dat",s->output,s->interface_tolerance,gr->fileCounter);
	gr->fp3=fopen(buffer,"w");
	sprintf(buffer,"%shigh_dev-%f-tol-%d.dat",s->output,s->interface_tolerance,gr->fileCounter);
	gr->fp4=fopen(buffer,"w");
	sprintf(buffer,"%sdev_deep-%f-tol-%d.dat",s->output,s->interface_tolerance,gr->fileCounter);

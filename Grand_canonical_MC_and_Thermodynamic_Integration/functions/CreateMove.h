...
void update(int par, struct SYSTEM *box,struct PARAINPUT *p)
{
	...
	{
//         if(par==2)
//             printf("updating neigh list par\n");
		box->maxdispsq=0.0;
		vnlist(&(*p),&(*box));
	}
}
...
double moveAtom(struct SYSTEM *box, struct PARAINPUT *p,int mc,struct APPOGGIO *a)
{
	...
	for(t=0;t<p->nAto;t++)
	{
		...
		Yold=box->Y[par];
		Zold=box->Z[par];
		ene_before=oldparenergy(par,&(*box),&(*p));	// Energy before displacement
...
//         printf("moving %d %d\n",par,t);
		...
		ene_after=newparenergy(par,&(*box),&(*p));	// Energy after displacement
//          printf("before after %f %f\n",ene_before,ene_after);
...
void moveVol(struct SYSTEM *structbox,struct PARAINPUT *p,struct APPOGGIO *a)
{
	...
	for(i=0;i<p->nAto;i++)				// Move the volume
	{

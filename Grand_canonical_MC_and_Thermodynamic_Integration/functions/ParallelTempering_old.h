void countList(struct PARAINPUT *p, double **source, int size)
{
	...
	for(i=1;i<size;i++)
	{
		if(n0==source[i][0] && rho==source[i][2])
		{
			p->nT++;
		}
	}
	...
	for(i=1;i<size;i++)
	{
		if(T==source[i][4] && rho==source[i][2])
		{
			p->nSp++;
		}
	}
...
{	
	...
	app->deltaBeta=(1.0/structbox->tem-1.0/source[proc][4]); // find out what this does
	...
	if(ran<exp(app->deltaBeta*(p->press*app->vold+(structbox->potEne-Uswap))))
	{
		app->esitoSwap=1;
...
//void Nl4SwapSend(struct SYSTEM *structbox, int proc,struct APPOGGIO *app)
//{
//	MPI_Send(&structbox->nl4_max,1,MPI_INT,proc,23,MPI_COMM_WORLD);									// tag 23 = n1
//	MPI_Recv(&app->esitoSwap,1,MPI_INT,proc,2,MPI_COMM_WORLD,MPI_STATUS_IGNORE);			// tag 2 = esitoSwap
//}
	
//void Nl4SwapRiceive(struct SYSTEM *structbox, int proc, double **source,struct APPOGGIO *app,struct PARAINPUT *p)
//{	
//	double ran,wN,w0;
//	int n1Swap;
//	
//	MPI_Recv(&n1Swap,1,MPI_INT,proc,23,MPI_COMM_WORLD,MPI_STATUS_IGNORE);		// tag 23 = n1
//	
//	wN=0.5*source[proc][4]*sqr(structbox->nl4_max-source[proc][3])+0.5*structbox->kNl4*sqr(n1Swap-structbox->nl4_max0);
//	w0=0.5*source[proc][4]*sqr(n1Swap-source[proc][3])+0.5*structbox->kNl4*sqr(structbox->nl4_max-structbox->nl4_max0);

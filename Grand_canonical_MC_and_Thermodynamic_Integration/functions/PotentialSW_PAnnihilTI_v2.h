    ...
    if(par==p->nAto+1)
    {
        for(j=1;j<=p->nAto;j++)
        {
            ...
            if(rijsq<sqr(SWcut))
            {
                ...
                box->Adhi[par]+=gij;
                box->AdSiX[par]+=diffX*gij/rij;
                box->AdSiY[par]+=diffY*gij/rij;
                box->AdSiZ[par]+=diffZ*gij/rij;
                box->AdTiXX[par]+=diffX*diffX*gij/sqr(rij);
                box->AdTiXY[par]+=diffX*diffY*gij/sqr(rij);
                box->AdTiXZ[par]+=diffX*diffZ*gij/sqr(rij);
                box->AdTiYY[par]+=diffY*diffY*gij/sqr(rij);
                box->AdTiYZ[par]+=diffY*diffZ*gij/sqr(rij);
                box->AdTiZZ[par]+=diffZ*diffZ*gij/sqr(rij);

                U2+=(SWaCon*((SWbCon*pow(rij,-4.0))-1.0)*exp(1.0/(rij-SWcut)));
                U3Correct+=(16.0*SWlamb/9.0)*sqr(gij);

                diffhisq=(2.0*box->Ahi[par]-box->Adhi[par])*box->Adhi[par];
                diffsisq=(2.0*box->AsiX[par]-box->AdSiX[par])*box->AdSiX[par]+(2.0*box->AsiY[par]-box->AdSiY[par])*box->AdSiY[par]+(2.0*box->AsiZ[par]-box->AdSiZ[par])*box->AdSiZ[par];
                diffTisqXX=(2.0*box->ATiXX[par]-box->AdTiXX[par])*box->AdTiXX[par]+(2.0*box->ATiXY[par]-box->AdTiXY[par])*box->AdTiXY[par]+(2.0*box->ATiXZ[par]-box->AdTiXZ[par])*box->AdTiXZ[par];
                diffTisqYY=(2.0*box->ATiXY[par]-box->AdTiXY[par])*box->AdTiXY[par]+(2.0*box->ATiYY[par]-box->AdTiYY[par])*box->AdTiYY[par]+(2.0*box->ATiYZ[par]-box->AdTiYZ[par])*box->AdTiYZ[par];
                diffTisqZZ=(2.0*box->ATiXZ[par]-box->AdTiXZ[par])*box->AdTiXZ[par]+(2.0*box->ATiYZ[par]-box->AdTiYZ[par])*box->AdTiYZ[par]+(2.0*box->ATiZZ[par]-box->AdTiZZ[par])*box->AdTiZZ[par];
                diffTrTisq = diffTisqXX + diffTisqYY + diffTisqZZ;

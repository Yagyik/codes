...
double energy(int flag,struct SYSTEM *box,struct PARAINPUT *p)
{
	...
	for(i=1;i<p->nAto;i++)
	{
		...
		// updating quantities for i->N+1 interactions (other than U2,U3 etc - only array quantities updated)
		neipar=p->nAto;
		diffX=box->X[neipar]-box->X[i];
        diffX-=box->lx*lround(diffX/box->lx);
        diffY=box->Y[neipar]-box->Y[i];
    ...
    en = U2+U3-U3correct;
    U2=U3=U3correct=0.0;
    // above en for standard interactions
    // now energy of new particle
    // update U2, U3 here and also array quantities for N+1 particle
    for(j=1;j<p->nAto;j++)
    {
        neipar=j;
        diffX=box->X[neipar]-box->X[p->nAto];
        diffX-=box->lx*lround(diffX/box->lx);
        diffY=box->Y[neipar]-box->Y[p->nAto];
        diffY-=box->ly*lround(diffY/box->ly);
        diffZ=box->Z[neipar]-box->Z[p->nAto];
        diffZ-=box->lz*lround(diffZ/box->lz);
        ...
        if(rijsq<sqr(SWcut))
        {
            ...
            box->hi[p->nAto]+=gij;
            box->siX[p->nAto]+=diffX*gij/rij;
            box->siY[p->nAto]+=diffY*gij/rij;
            box->siZ[p->nAto]+=diffZ*gij/rij;
            box->TiXX[p->nAto]+=diffX*diffX*gij/sqr(rij);
            box->TiXY[p->nAto]+=diffX*diffY*gij/sqr(rij);
            box->TiXZ[p->nAto]+=diffX*diffZ*gij/sqr(rij);
            box->TiYY[p->nAto]+=diffY*diffY*gij/sqr(rij);
            box->TiYZ[p->nAto]+=diffY*diffZ*gij/sqr(rij);
            box->TiZZ[p->nAto]+=diffZ*diffZ*gij/sqr(rij);
        }
    }


    sisq=sqr(box->siX[p->nAto])+sqr(box->siY[p->nAto])+sqr(box->siZ[p->nAto]);
    TisqXX=sqr(box->TiXX[p->nAto])+sqr(box->TiXY[p->nAto])+sqr(box->TiXZ[p->nAto]);
    TisqYY=sqr(box->TiXY[p->nAto])+sqr(box->TiYY[p->nAto])+sqr(box->TiYZ[p->nAto]);
    TisqZZ=sqr(box->TiXZ[p->nAto])+sqr(box->TiYZ[p->nAto])+sqr(box->TiZZ[p->nAto]);
		
    U3+=SWlamb/18.0*sqr(box->hi[p->nAto])+SWlamb/3.0*sisq+SWlamb/2.0*(TisqXX+TisqYY+TisqZZ);
    en + = box->lambda_n*(U2+U3-U3correct);
	if(flag==1)
	{
		box->virial=0.0;
		for(i=1;i<=p->nAto;i++)
...
double oldparenergy(int par, struct SYSTEM *box)
{
    ...
    // if par = p->nAto then finish off calc and return here
    if(par==p->nAto)
    {
        for(j=1;j<p->nAto;j++)
        {
    ...
    else // for the N+1 particle
    {
        ...
        dUSi=(double)(SWlamb/3.0)*sisq;
        dUTi=(double)(SWlamb/2.0)*(TisqXX+TisqYY+TisqZZ);
        // now calculate for particle with N+1
        if(par!=p->nAto)
        {
        neig=p->nAto;
        diffX=box->X[neig]-box->X[par];
...
double newparenergy(int par, struct SYSTEM *box)
{
    ...
    else // par!=nAto
    {
        ...
        // now calculate for interaction with the N+1 particle
        // first reset accumulators

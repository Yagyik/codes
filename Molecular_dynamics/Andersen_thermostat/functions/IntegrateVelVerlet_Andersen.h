...
void integrateVelVerlet(struct SYSTEM *box,struct PARAINPUT *p,struct APPOGGIO *a, int step)
{
...
//             printf("%f %f %f\n",rand,p->nu,p->dt);
            if(rand<p->nu*p->dt)
            {
//                 printf("resampling vel for %d\n",i);
                box->vx[i]=gauss_var_AT(0,sig,&(*a));
                box->vy[i]=gauss_var_AT(0,sig,&(*a));
                box->vz[i]=gauss_var_AT(0,sig,&(*a));

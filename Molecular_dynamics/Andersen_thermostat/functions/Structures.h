...
{
	// Thermo variables
	double tem,potEne,KE,totEne,pressure,rho;
	double lx,ly,lz;
...
//         structbox->rho0=0.455;
    ...
    // for bcc/fcc
    // 4 for fcc
    structbox->xunit=calloc(4,sizeof(double *));
	structbox->yunit=calloc(4,sizeof(double *));
	structbox->zunit=calloc(4,sizeof(double *));
    printf("%f\n",alat);
    structbox->xunit[0]=0.0*alat;
	structbox->yunit[0]=0.0*alat;
	structbox->zunit[0]=0.0*alat;
    structbox->xunit[1]=0.5*alat;
	structbox->yunit[1]=0.5*alat;
	structbox->zunit[1]=0.0*alat;
    structbox->xunit[2]=0.5*alat;
	structbox->yunit[2]=0.0*alat;
	structbox->zunit[2]=0.5*alat;
    structbox->xunit[3]=0.0*alat;
	structbox->yunit[3]=0.5*alat;
	structbox->zunit[3]=0.5*alat;
...
//     structbox->nlen=p->nAto+1;
    ...
    structbox->oldfx=calloc(p->nAto,sizeof(double *));
    structbox->oldfy=calloc(p->nAto,sizeof(double *));
    structbox->oldfz=calloc(p->nAto,sizeof(double *));

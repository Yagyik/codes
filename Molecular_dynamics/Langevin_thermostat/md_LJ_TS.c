...
#include "functions/Integrate_Langevin.h"
...
/********************************************************************* INIZIO CODICE MONTE CARLO **************************************************/
    ...
    for(mdstep=mdstart;mdstep<=p.totRun;mdstep++)
    {
        if(mdstep%p.thermo==0)									// Thermo output
		{
            ...
            for(i=0;i<p.nAto;i++)
            {
//                 for(int j=0;j<p.nAto;j++)
//                 {
//                 
//                 if(j>i)
//                 {
//                 spamx=structbox.X[i]-structbox.X[j];
//                 spamy=structbox.Y[i]-structbox.Y[j];
//                 spamz=structbox.Z[i]-structbox.Z[j];
//                 spamx-=structbox.lx*lround(spamx/structbox.lx);
//                 spamy-=structbox.ly*lround(spamy/structbox.ly);
//                 spamz-=structbox.lz*lround(spamz/structbox.lz);
//                 spamx=structbox.X[i]-structbox.lx*lround(structbox.X[i]/structbox.lx);
//                 spamy=structbox.Y[i]-structbox.ly*lround(structbox.Y[i]/structbox.ly);
//                 spamz=structbox.Z[i]-structbox.lz*lround(structbox.Z[i]/structbox.lz);
//                 spamx=structbox.X[i]+0.5*structbox.lx;
//                 spamy=structbox.Y[i]+0.5*structbox.lx;
//                 spamz=structbox.Z[i]+0.5*structbox.lx;
                spamx=structbox.X[i];
                spamy=structbox.Y[i];
                spamz=structbox.Z[i];
                rsq=spamx*spamx+spamy*spamy+spamz*spamz;
                nx=1;
                ny=1;
                nz=-1;
//                 for(nx=-1;nx<=1;nx+=2)
...
//                             if(nx+ny+nz==1)
//                             {
                                if(rsq>0.0 && rsq<0.25*structbox.lx)
                                {
                                fact=(2.0*M_PI/(0.25*structbox.lx*sqrt(rsq)))*(1.0*nx*spamx+1.0*ny*spamy+1.0*nz*spamz);
//                                 printf("spam %f %f %f\n",spamx,spamy,spamz);
//                                 printf("n %d %d %d k %f\n",nx,ny,nz,2.0*M_PI*sqrt(3.0)/(0.25*structbox.lx));
//                                 printf("%f\n",(nx*spamx+ny*spamy+nz*spamz));
//                                  printf("fact %f %f\n",fact,cos(fact));
                                rhok+=cos(fact);
                                denomrhok+=1.0;
                                }
...
//             printf("done thermo printing\n");
        ...
        //integrateVelVerlet(&structbox,&p,&app,0); // 0 means updated positions and stored old forces
        integrateLangevin(&structbox,&p,&app);
        structbox.potEne=energyForce(&structbox,&p);
        //integrateVelVerlet(&structbox,&p,&app,1); // update velocity using old force and new force
        kinEnergy(&structbox,&p);
        structbox.totEne = structbox.potEne + structbox.KE;

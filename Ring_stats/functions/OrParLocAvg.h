...
void Init_OrParla(SIMDAT *s, DATASIM *d, ORDER *op)
{
	int i,j,l,m;
	op->maxCoord = 30;
	op->counter=0;
...
void compute_fla(SIMDAT *s, DATASIM *d, ORDER *op)
{
...
void qlmla(DATASIM *d, ORDER *op, int a1, int a2)
{
	int l,m;
...
void print_OrParla(SIMDAT *s, DATASIM *d, ORDER *op)
{
	FILE *fp;	
	int t,l;
	double sx,sy,sz;
	...
	sprintf(buffer,"%sQlocAvgDist%g-ra%g-%d.dat",s->output,s->laOrPar_r,s->la_OrPar_ra,op->fileCounter);
  	fp=fopen(buffer,"w"); 
...
void Compute_OrParla(SIMDAT *s, DATASIM *d, ORDER *op)
{
	int a1,a2,l,m,mspam;

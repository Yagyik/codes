struct SPRING
{
	...
	int *visited,*inpath,*complete,*pathstack,*hitlist,*ringcount, *neigh, *atoTotRing;
...
void print_spring_3d(SIMDAT *s, DATASIM *d, SPRING *sp)
{
	...
	{
		fprintf(fp,"%d %d\n",t,sp->atoTotRing[t]);
		fprintf(fp2,"SC%d %f %f %f\n",sp->atoTotRing[t],d->r[t][0],d->r[t][1],d->r[t][2]);
	}
	fclose(fp);
	fclose(fp2);
